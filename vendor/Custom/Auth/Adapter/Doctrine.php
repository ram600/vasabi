...
class Doctrine implements \Zend\Authentication\Adapter\AdapterInterface
{
    /**
    ...
    /**
     * hash foo for Credential values
     * 
     * @var mixed
     */
    protected $_hashFoo = null;


    
    
    protected $_resultEntity;
    /**
     ...
     * @param  mixed(Closure must return credential or some exist foo) $hashFoo
     * @return void
     */
    public function __construct($em = null, $entityName = null, $identityColumn = null,
                                $credentialColumn = null,$hashFoo = null)
    {
        if (null !== $em) {
            $this->setEm($em);
        }
    ...
    /**
     * setHashFoo - set hash funciton for password
     * @param Closure|string
     */
    public function setHashFoo($foo){
        $this->_hashFoo = $foo;
    }
    /**
    ...
    {
         if(is_string($this->_hashFoo) && function_exists($this->_hashFoo)){
                $credential = call_user_func($this->_hashFoo, $credential);
         }elseif($this->_hashFoo instanceof \Closure){
                $foo = $this->_hashFoo;
    ...
    protected function _performQuery(\Doctrine\ORM\Query $query)
    {
        try {
             ...
             */
            require_once 'Zend/Auth/Adapter/Exception.php';
            throw new \Zend\Authentication\Adapter\Exception\RuntimeException('The supplied parameters to \Doctrine\ORM\EntityManager failed to '
    ...
    protected function _validateResult($resultIdentities)
    {
        ...
        if (count($resultIdentities) < 1) {
            $this->_authenticateResultInfo['code'] = \Zend\Authentication\Result::FAILURE_IDENTITY_NOT_FOUND;
            $this->_authenticateResultInfo['messages'][] = 'A record with the supplied identity could not be found.';
            return $this->_authenticateCreateAuthResult();
        } elseif (count($resultIdentities) > 1) {
            $this->_authenticateResultInfo['code'] = \Zend\Authentication\Result::FAILURE_IDENTITY_AMBIGUOUS;
            $this->_authenticateResultInfo['messages'][] = 'More than one record matches the supplied identity.';
            return $this->_authenticateCreateAuthResult();
        } elseif (count($resultIdentities) == 1) {
        	$resultIdentity = $resultIdentities[0];
            if ($resultIdentity->{$this->_credentialColumn} != $this->_credential) {
            	$this->_authenticateResultInfo['code'] = \Zend\Authentication\Result::FAILURE_CREDENTIAL_INVALID;
                $this->_authenticateResultInfo['messages'][] = 'Supplied credential is invalid.';
            } else {
            	$this->_authenticateResultInfo['code'] = \Zend\Authentication\Result::SUCCESS;
        	...
        	$this->_authenticateResultInfo['code'] = \Zend\Authentication\Result::FAILURE_UNCATEGORIZED;
        }
